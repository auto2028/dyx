...
export default {
    async fetch(request, env) {
        ...
        proxyIP = env.PROXYIP || env.proxyip || proxyIP; // 从环境变量获取PROXYIP
        proxyIPs = proxyIP ? proxyIP.split(',') : []; // 处理多代理IP，支持逗号分隔的IP列表
        ...
        if (!([mytoken, fakeToken, 访客订阅].includes(token) || url.pathname == ("/" + mytoken) || url.pathname.includes("/" + mytoken + "?"))) {
            ...
            if (proxyIPs.length > 0) {
                return await proxyURL(proxyIPs, url);
            } else if (env.URL302) {
